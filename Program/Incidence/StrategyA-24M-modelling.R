##Step1##
##train BRT models using one-year sample under strategy a+##
##Author:Fangyu Ding##
##Time:20200716##
...
for(p in 1:length(years)){
  tempyear = years[p]
  filename=paste("E:\\Conflict\\Tables\\RevisedVersion\\Precipitation\\24MonthsSD\\",years[p],".csv",sep='')
  ##load SPI24 dataset
  dataPrecipitation <- read.csv(filename,header=T,encoding="utf-8")
  names(dataPrecipitation) <- c("nrow","ncol","SPI24")
  
  filename=paste("E:\\Conflict\\Tables\\RevisedVersion\\Temperature\\24MonthsSD\\",years[p],".csv",sep='')
  ##load STI24 dataset
  dataTemperature <- read.csv(filename,header=T,encoding="utf-8")
  names(dataTemperature) <- c("nrow","ncol","STI24")
  ...
  Alldata<- subset(Alldata,SPI24 > -100)
  Alldata<- subset(Alldata,STI24 > -100)
  ...
  for(i in 1 : 20 ){
    ...
    filename=paste("E:\\Conflict\\Tables\\Samples-Revisedversion\\",years[p],"_",i,"_StrategyA_24M_samples.csv",sep='')
    ...
    tempdata <- subset(tempdata, select = -EthnicDiversity)
    library(car)
    ...
    BRT_model <- gbm.step(data=training, gbm.x = 1:10, gbm.y = 12, tree.complexity = 4,learning.rate = 0.01, bag.fraction = 0.75, step.size = 10, cv.folds = 10, max.trees = 10000)
    ...
    modelname<-paste("E:\\Conflict\\Model\\",years[p],"_",i,"_BRT_StrategyA_24M.Rdata",sep="")
    ...
    simulate_filename <- paste("E:\\Conflict\\Tables\\Simulation\\StrategyA_24M_ConflictRisk_simulated_",years[p],"_",i,".csv",sep="")
    write.csv(copy_Alldata, file=simulate_filename,row.names=F)
  }
  filename<-paste("E:\\Conflict\\Tables\\Accuracy\\StrategyA_24M_accuracy_",years[p],".csv",sep="")
...
for(ModelYear in 2000:2015){
  ...
  for(SampleYear in 2000:2015){
    for(Modelorder in 1:20){
      modelname<-paste("E:\\Conflict\\Model\\",ModelYear,"_",Modelorder,"_BRT_StrategyA_24M.Rdata",sep="")
      load(modelname)
      
      filename=paste("E:\\Conflict\\Tables\\Samples-Revisedversion\\",SampleYear,"_",Modelorder,"_StrategyA_24M_samples.csv",sep='')
      ...
      isPositiveClass <- tempsamples[,12] == 1
  ...
  save_file<-paste("E:\\Conflict\\Tables\\Performace\\StrategyA_24M_accuracy_",ModelYear,".csv",sep="")
...
filename=paste("E:\\Conflict\\Tables\\Performace\\StrategyA_24M_accuracy_",2000,".csv",sep="")
##load background dataset
data <- read.csv(filename,header=T,encoding="utf-8")
for(ModelYear in 2001:2015){
  filename=paste("E:\\Conflict\\Tables\\Performace\\StrategyA_24M_accuracy_",ModelYear,".csv",sep="")
  
  tempdata <- read.csv(filename,header=T,encoding="utf-8")
  data <- rbind(data, tempdata)
}
data <- subset(data, select = -X)
write.csv(data, file = "E:\\Conflict\\Tables\\Performace\\StrategyA_24M_accuracy.csv")

##Calculate 95%CI ##
data <- read.csv(file = "E:\\Conflict\\Tables\\Performace\\StrategyA_24M_accuracy.csv",header=T,encoding="utf-8")
Accuracy_data <- c()
for(ModelYear in 2000:2015){
  for(SampleYear in 2000:2015){
...
###For strategy a+, Generate the data source of Fig. 1, Supplementary Fig. 2 and Supplementary Table 1###
write.csv(Accuracy_data, file="E:\\Conflict\\Tables\\Performace\\Performance_Timecorss_StrategyA_24M.csv",row.names = FALSE,col.names = FALSE,quote = F)
###End###


##Step3##
##train BRT models using the merged all samples (2000-2015) under strategy a+##
##Author:Fangyu Ding##
##Time:20200716##
...
timestart<-Sys.time();
for(order in 1:20){
  filename=paste("E:\\Conflict\\Tables\\Samples-Revisedversion\\2000_",order,"_StrategyA_24M_samples.csv",sep="")
  ##load background dataset
  data <- read.csv(filename,header=T,encoding="utf-8")
  for(ModelYear in 2001:2015){
    filename=paste("E:\\Conflict\\Tables\\Samples-Revisedversion\\",ModelYear,"_",order,"_StrategyA_24M_samples.csv",sep="")
    tempdata <- read.csv(filename,header=T,encoding="utf-8")
    data <- rbind(data, tempdata)
  }
  filename=paste("E:\\Conflict\\Tables\\Samples-Revisedversion\\StrategyA_24M_AllSamples_",order,".csv",sep="")
...
for(i in 1 : 20 ){
  ...
  temp_temp_data <- Alldata[,6]
  Alldata$EthnicDiversity <- as.factor(temp_temp_data)  
  ...
  modelname<-paste("E:\\Conflict\\Model\\BRT_StrategyA_24M_AllSample_",i,".Rdata",sep="")
  
  save(BRT_model, file = modelname)
}
filename<-paste("E:\\Conflict\\Tables\\Accuracy\\StrategyA_24M_AllSample_accuracy.csv",sep="")
write.csv(Accuracy_data, file=filename)
...
##Step4##
##Simulated the risk of armed conflicts at global scale using the BRT models trained on all samples (2000-2015) under strategy a+ ##
##Author:Fangyu Ding##
##Time:20200716##
...
for(p in 1:length(years)){
  ...
  for(i in 1 : 20 ){
    modelname <-
      paste("E:\\Conflict\\Model\\BRT_StrategyA_24M_AllSample_",
            i,
            ".Rdata",
            sep = "")
    load(modelname)
    ...
    copy_Alldata$Risk <- pred
    simulate_filename <- paste("E:\\Conflict\\Tables\\Simulation\\All_StrategyA_24M_ConflictRisk_simulated_",years[p],"_",i,".csv",sep="")
    write.csv(copy_Alldata, file=simulate_filename,row.names=F)
  }
}
timeend<-Sys.time()
